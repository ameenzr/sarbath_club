...
19 September mobile design: owner requested a very minimal interface optimized for phones. Replaced the large decorative hero/two-column layout with a compact single column across customer/staff/privacy. Use local system fonts, readable 16 px inputs, large primary buttons and safe-area spacing. Verification chooses compact below 300 px available width, flexible otherwise, per [Cloudflare widget sizing](https://developers.cloudflare.com/turnstile/get-started/client-side-rendering/widget-configurations/). Verification still appears only at claim.

Owner-requested change: customers play first without entering name/mobile or completing verification. Only after winning (120–449 ms reaction), the customer enters identity and completes Turnstile verification to claim a coupon via `/api/claim`. Unlimited plays with immediate replay on loss. Each phone may hold one unredeemed coupon at a time (7-day validity from claim issuance); redemption immediately releases the phone for another claim. Migration 0005 adds `plays`, `play_sessions`, `coupons`, and `coupon_locks` tables; legacy `attempts`/`sessions` remain for compatibility. Backend `reserve` is now anonymous (no name/phone), and `claim` is a separate step.
