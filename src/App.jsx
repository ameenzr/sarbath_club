...
function Brand() { return <a className="brand" href="/"><img src="/logo.png" width="32" height="32" alt="" /><span>Sarbath Club</span></a>; }
function Shell({ children, staff = false }) {
  return <div className="site"><header><Brand /><a className="nav-link" href={staff ? '/' : '/staff'}>{staff ? 'Play' : 'Staff'}</a></header>{children}<footer><a href="/privacy">Privacy</a><span>No purchase needed</span></footer></div>;
}
...
function Game() {
  ...
  return <Shell><main className="game-layout"><section className="intro"><h1>Quick sip challenge</h1><p>120–449 ms to win · Unlimited plays</p></section><section className="game-card" aria-live="polite">
    {screen === 'welcome' && <><h2>Tap fast. Sip free.</h2><p>Wait for blue, then tap anywhere.<br />Win first. Enter your details after.</p>{config && <div className="prize-strip"><strong>{config.prize.label}</strong><small>Coupon valid for 7 days.</small></div>}<button className="primary" disabled={!config?.enabled} onClick={start}>Let's play</button><div className="card-foot">One coupon per phone every 7 days</div></>}
    ...
    {screen === 'claim' && <><h2>Claim your coupon</h2><p>You won. Add your details to claim.</p>{data?.reactionMs !== null && <div className="time-value">{data.reactionMs}<span>ms · estimated reaction</span></div>}<form onSubmit={submitClaim}><label htmlFor="name">Your name</label><input id="name" autoComplete="given-name" required maxLength={80} value={name} onChange={e => setName(e.target.value)} placeholder="Name" /><label htmlFor="phone">Mobile number</label><div className="phone-input"><span>+91</span><input id="phone" type="tel" inputMode="tel" autoComplete="tel-national" required pattern="[6-9][0-9]{9}" maxLength={10} value={phone} onChange={e => setPhone(e.target.value)} placeholder="10-digit mobile" /></div><Turnstile onToken={setVerification} generation={generation} /><button className="primary" disabled={!verification}>Claim reward</button><p className="privacy-note">We use your name and phone only to issue and verify your coupon. <a href="/privacy">Privacy details</a></p></form></>}
    {screen === 'result' && <div className="result-screen"><div className="eyebrow">{data.status === 'won' ? 'YOU DID IT!' : 'THANKS FOR PLAYING'}</div><h2 className="result-title">{resultTitle[data.status]}</h2>{data.reactionMs !== null && <div className="time-value">{data.reactionMs}<span>ms · estimated reaction</span></div>}{data.status === 'won' ? <><p>You earned <strong>{data.prize.label}</strong>.</p><div className="coupon"><span>Coupon code</span><strong>{data.code}</strong><small>{data.prize.terms}</small><small>Valid until {new Date(data.expiresAt).toLocaleString('en-IN')}</small></div><p>Show this code at the counter.</p><button className="primary" onClick={newPlay}>Play again</button></> : <><p>{data.status === 'too_early' ? 'You tapped before the blue screen appeared or faster than the eligible 120 ms minimum.' : data.status === 'expired' ? 'This play expired before you tapped.' : 'Keep those reflexes fresh.'}</p><button className="primary" onClick={newPlay}>Play again</button></>}<div className="card-foot">One coupon per phone every 7 days</div>{isDiagnostic && diag.current && <pre className="diagnostic">{JSON.stringify(diag.current, null, 2)}</pre>}</div>}
    {screen === 'active_coupon' && <div className="status-screen"><h2>Coupon already active</h2><p>Claim again after your current coupon expires. Redeeming it sooner does not reset the 7 days.</p><button className="primary" onClick={newPlay}>Play again</button></div>}
    {screen === 'interrupted' && <div className="status-screen"><div className="status-symbol">◷</div><h2>Recover your play</h2><p>Check your saved result. Staff can also look up a claimed coupon.</p><button className="primary" onClick={recover}>Check result</button></div>}
