...
function Game() {
  const [screen, setScreen] = useState('form'), [name, setName] = useState(''), [phone, setPhone] = useState(''), [verification, setVerification] = useState('');
  ...
  useEffect(() => {
    ...
    if (stored?.credential) { credential.current = stored.credential; setScreen('recovering'); api('result', undefined, stored.credential).then(r => { setData(r); setScreen(r.status === 'reserved' ? 'interrupted' : 'result'); }).catch(e => { setError(e.message); setScreen('interrupted'); }); }
  ...
  async function start(event) {
    ...
    try {
      ...
      const result = await api('flash', { name, phone, turnstileToken: verification }, credential.current);
      setData(result);
      if (result.status !== 'reserved') { setScreen('result'); return; }
      ...
      else { if (e.key !== 'already_played') { sessionStorage.removeItem('sarbath-play'); credential.current = null; } setScreen(e.key === 'already_played' ? 'already_played' : 'form'); setVerification(''); setGeneration(x => x + 1); }
  ...
  async function tap(early = false) {
    ...
    try { const r = await api('tap', { early }, credential.current); setData(r); diag.current = { samples: samples.current, localMs, serverMs: r.reactionMs, differenceMs: localMs === null ? null : r.reactionMs - localMs }; setScreen('result'); }
    catch (e) { setError(e.message); setScreen('interrupted'); }
    finally { lock.current = false; }
  }
  async function recover() {
    if (lock.current) return; lock.current = true; setError('');
    try { const r = await api('result', undefined, credential.current); setData(r); setScreen(r.status === 'reserved' ? 'interrupted' : 'result'); if (r.status === 'reserved') setError('This play is still pending. Wait a few seconds, then check again.'); }
    catch (e) { setError(e.message); if (e.key === 'invalid_session') { setScreen('form'); setVerification(''); setGeneration(x => x + 1); setError('No reserved play was found. Re-enter your details to retry the same Start request.'); } } finally { lock.current = false; }
  }
  ...
  return <Shell><main className="game-layout"><section className="intro"><div className="eyebrow"><span className="live-dot" /> THE QUICK SIP CHALLENGE</div><h1>THINK<br />FAST.<br /><span>SIP HAPPY.</span></h1><p className="lede">A little challenge. A refreshing reward.<br />How quick are your reflexes?</p><div className="intro-stamp">120–449<span>ms to win</span></div><div className="how"><span>01 <b>Get ready</b></span><span>02 <b>Wait for blue</b></span><span>03 <b>Tap fast</b></span></div></section><section className="game-card" aria-live="polite">
    {screen === 'form' && <><div className="card-top"><span>YOUR NEXT SIP STARTS HERE</span><span>↯</span></div><h2>Ready, steady,<br /><em>refresh.</em></h2><p>Enter your details, wait for the screen to turn blue, then tap. A quick reaction earns a reward.</p>{config && <div className="prize-strip"><span>THE REWARD</span><strong>{config.prize.label}</strong><small>{config.prize.terms}</small></div>}<form onSubmit={start}><label htmlFor="name">Your name</label><input id="name" autoComplete="given-name" required maxLength={80} value={name} onChange={e => setName(e.target.value)} placeholder="What should we call you?" /><label htmlFor="phone">Mobile number</label><div className="phone-input"><span>+91</span><input id="phone" type="tel" inputMode="tel" autoComplete="tel-national" required pattern="[6-9][0-9]{9}" maxLength={10} value={phone} onChange={e => setPhone(e.target.value)} placeholder="10-digit mobile number" /></div><Turnstile onToken={setVerification} generation={generation} /><button className="primary" disabled={!verification || !config?.enabled}>LET'S PLAY <span>↗</span></button><p className="privacy-note">We use your name and phone to enforce one play per day and verify your prize at pickup. <a href="/privacy">Privacy details</a></p></form><div className="card-foot">ONE PLAY PER PHONE, PER DAY · NO PURCHASE TO PLAY</div></>}
    {(screen === 'checking' || screen === 'submitting' || screen === 'recovering') && <div className="status-screen"><div className="status-symbol">◷</div><h2>{screen === 'checking' ? 'Checking your connection' : screen === 'submitting' ? 'A moment for your result' : 'Finding your play'}</h2><p>Your play stays safe if the connection is interrupted.</p></div>}
    {screen === 'result' && <div className="result-screen"><div className="eyebrow">{data.status === 'won' ? 'YOU DID IT!' : 'THANKS FOR PLAYING'}</div><h2 className="result-title">{resultTitle[data.status]}</h2>{data.reactionMs !== null && <div className="time-value">{data.reactionMs}<span>ms · estimated reaction</span></div>}{data.status === 'won' ? <><p>You earned <strong>{data.prize.label}</strong>.</p><div className="coupon"><span>YOUR WINNING CODE</span><strong>{data.code}</strong><small>{data.prize.terms}</small><small>Valid until {new Date(data.expiresAt).toLocaleString('en-IN')}</small></div><p>Show this code to staff. They’ll confirm it at the counter.</p></> : <p>{data.status === 'too_early' ? 'You tapped early or faster than the eligible range. Today’s play is used.' : data.status === 'expired' ? 'This play expired. Today’s play is used.' : 'Keep those reflexes fresh.'} Come back tomorrow for another try.</p>}<div className="card-foot">ONE PLAY PER PHONE, PER DAY</div>{isDiagnostic && diag.current && <pre className="diagnostic">{JSON.stringify(diag.current, null, 2)}</pre>}</div>}
    {(screen === 'already_played' || screen === 'interrupted') && <div className="status-screen"><div className="status-symbol">◷</div><h2>{screen === 'already_played' ? 'See you tomorrow.' : 'Let’s find your result.'}</h2><p>{screen === 'already_played' ? 'You’ve already played today. Tomorrow brings another refreshing chance.' : 'Your reserved play stays the same. Recover it here, or ask staff to look up an issued coupon. Refreshing does not start another play.'}</p>{screen === 'interrupted' && <button className="primary" onClick={recover}>CHECK THIS PLAY ↗</button>}</div>}
...
function Privacy() {
  ...
  return <Shell><main className="privacy-page"><div className="eyebrow">YOUR DETAILS</div><h1>A LITTLE<br /><span>PRIVACY NOTE.</span></h1><section className="staff-card"><h2>What we collect</h2><p>Your name, mobile number, play date, estimated reaction result, and any coupon/redemption record. We use them to enforce daily eligibility and verify prizes. We do not use them for marketing.</p><h2>How long we keep it</h2><p>{config ? `We retain your details for ${config.retentionDays} days after your play, then remove them automatically during our regular cleanup.` : 'Your details are retained for 30 days after your play, then removed automatically during our regular cleanup.'}</p><h2>Who can see it</h2><p>Authorized staff can search coupons. The owner can manage and export operational records. Cloudflare provides hosting, database, and bot verification infrastructure.</p><h2>Browser storage</h2><p>A temporary session credential lets this tab recover your result. Short-lived security cookies support connection checks and staff access. No third-party marketing analytics are installed.</p><a href="/">Back to the challenge ↗</a></section></main></Shell>;
}
