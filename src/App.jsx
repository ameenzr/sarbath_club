...
function Shell({ children, staff = false }) {
  return <div className="site"><header><Brand staff={staff} />{staff && <a className="nav-link" href="/">Let’s play!🧋</a>}</header>{children}<footer><a href="/privacy">Privacy</a></footer></div>;
}
...
function Game({ onGames }) {
  ...
  return <Shell><main className="game-layout"><button className="back-to-games" onClick={onGames}><span aria-hidden="true">▦</span>All games</button><section className="intro"><h1>Tap fast. Sip free.</h1><p>120–449 ms to win · Unlimited plays</p></section><section className="game-card" aria-live="polite">
    {screen === 'welcome' && <><h2>Ready to play?</h2><p>Wait for blue, then tap anywhere.<br />Win first. Enter your details after.</p>{config && <div className="prize-strip"><strong>{config.prize.label}</strong><small>Coupon valid for 7 days.</small></div>}<button className="primary" disabled={!config?.enabled} onClick={start}>Let's play</button><div className="card-foot">One unredeemed coupon per phone</div></>}
    ...
    {screen === 'claim' && null}
    {screen === 'result' && <div className="result-screen"><div className="eyebrow">{data.status === 'won' ? 'YOU DID IT!' : 'THANKS FOR PLAYING'}</div><h2 className="result-title">{resultTitle[data.status]}</h2>{data.reactionMs !== null && <div className="time-value">{data.reactionMs}<span>ms · estimated reaction</span></div>}{data.status === 'won' ? <><div className="coupon-ticket-intro"><span className="coupon-cup" aria-hidden="true">🧋</span><div><span>Your reward</span><b>{data.prize.label}</b></div></div><div className="coupon coupon-ticket"><span>COUPON CODE</span><div className="coupon-code"><strong>{data.code}</strong><button type="button" onClick={copyCoupon} aria-label={copied ? 'Coupon code copied' : 'Copy coupon code'} title={copied ? 'Copied' : 'Copy coupon code'}>{copied ? <span aria-hidden="true">✓</span> : <svg aria-hidden="true" viewBox="0 0 24 24"><rect x="8" y="8" width="11" height="11" rx="2" /><path d="M16 8V6a2 2 0 0 0-2-2H6a2 2 0 0 0-2 2v8a2 2 0 0 0 2 2h2" /></svg>}</button></div><div className="coupon-divider" /><div className="coupon-details"><div className="coupon-detail"><span aria-hidden="true">✓</span><div><b>Redeem at the counter</b><small>Show this code to staff.</small></div></div><div className="coupon-detail"><span aria-hidden="true">◷</span><div><b>Valid for 7 days</b><small>Until {new Date(data.expiresAt).toLocaleString('en-IN')}</small></div></div></div></div><a className="store-location" href="https://maps.app.goo.gl/wRiGJTGCmUnWQVrq9" target="_blank" rel="noreferrer"><span aria-hidden="true">⌖</span><span>Sarbath Club store location</span></a><button className="primary" onClick={newPlay}>Play again</button></> : <><p>{data.status === 'too_early' ? 'You tapped before the blue screen appeared or faster than the eligible 120 ms minimum.' : data.status === 'expired' ? 'This play expired before you tapped.' : 'Keep those reflexes fresh.'}</p><button className="primary" onClick={newPlay}>Play again</button></>}<div className="card-foot">One unredeemed coupon per phone</div>{isDiagnostic && diag.current && <pre className="diagnostic">{JSON.stringify(diag.current, null, 2)}</pre>}</div>}
